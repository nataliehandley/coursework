...
export const totalScoresArr = (scoresArr) => {
  const totalScores = scoresArr.reduce((total, amount) => {
    return total + amount;
  })
  return totalScores;
};
...
export const reverseString = (toReverse) => {
  const stringToArray = toReverse.split('');
...
export const sortCharactersAlphabetically = (charcterArr) => {
  let lowerCaseArray = charcterArr.map((character) => character.toLowerCase());
...
export const sortNumbersHighToLow = (numberArr) => {
  const sortNumbers = numberArr.sort((a, b) => b - a);
  return sortNumbers;
};
...
export const checkItemInstock = (toCheck) => {
  ...
  const indexItem = stockList.indexOf(toCheck);

  let message = `Sorry ${toCheck} is not instock.`;
  
  if (indexItem >= 0) {
    message = `${toCheck} is instock, it is on aisle ${indexItem}.`
}
return message;
...
export const checkPrimaryColours = (coloursArr) => {
  ...
  const primaryColourArray = coloursArr.forEach((color) => {
    if (color === primaryColours[i]) {
      return true;
    }
  })
