...
export const createListOfPoessessions = (possessionsArr, name) => {
  const namePossesions = possessionsArr.map((item) => {
    return name + " " + item;
  })
  return namePossesions;
};
...
export const convertStringToNumbersArray = (numberString) => {
  const plus = "+";
  let newNumberArray = numberString.split(plus);
  newNumberArray = newNumberArray.map(Number);
...
export const createOddEvenArray = (numberString) => {
  const plus = "+";
  let numberArray = numberString.split(plus);
  numberArray = numberArray.map(Number);
  let oddEven = numberArray.map((item) => {
    if (item % 2 == 0) {
      return "even";
    } else {
      return "odd";
...
export const filterBooksBySearch = (booksArr, searchTerm) => { //needed to use filter.(book) istead of of filter.(searchTerm) and then book.includes(searchTerm)
  const filteredList = booksArr.filter((book) => {
...
export const formatString = (string) => {
  const lettersArray = [];
  lettersArray = string.filter((letters) => {
    if(letters === /[a-zA-Z]/) {
      return true;
    } else {
      return false;
