...
export const moveFirstAndLastItems = (itemsArr) => {
  let result = [...itemsArr]; //missed this bit out, had to look at solution
...
export const removeEvenNumbers = (numberArr) => {
  ...
  let oddNumbers = numberArr2.filter((number) => {
    if (number % 2 !== 0) {
      return true;
    }else {
      return false;
...
export const reverseOrder = (toReverseArr) => {
  let toReverseArr2 = [...toReverseArr];
  let finalArray = toReverseArr2.reverse();
  return finalArray;
};
...
export const generateHighscores = (playersArr, scoresArr) => {
  let newArray = playersArr.map((index))
  return;
};
